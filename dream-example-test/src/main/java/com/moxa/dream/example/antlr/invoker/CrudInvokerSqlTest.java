...
//        DMLStatement dmlStatement = createStatement("@limit(`select 1 from dual order by a`,10,11)", null);
        try {
            ...
            System.out.println("\nmysql:" + new ToMYSQL().toStr(createStatement("@limit(`select 1 from dual order by a`,10,11)", null), new Assist(invokerFactoryList,null), null));
            System.out.println("\npgsql:" + new ToPGSQL().toStr(createStatement("@limit(`select 1 from dual order by a`,10,11)", null), new Assist(invokerFactoryList,null), null));
            System.out.println("\nmssql:" + new ToMSSQL().toStr(createStatement("@limit(`select 1 from dual order by a`,10,11)", null), new Assist(invokerFactoryList,null), null));
            System.out.println("\noracle:" + new ToORACLE().toStr(createStatement("@limit(`select 1 from dual order by a`,10,11)", null), new Assist(invokerFactoryList,null), null));
        } catch (InvokerException e) {
            throw new RuntimeException(e);
        }
    }
