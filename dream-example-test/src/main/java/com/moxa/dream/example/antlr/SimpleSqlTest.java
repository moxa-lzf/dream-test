package com.moxa.dream.example.antlr;
...
public class SimpleSqlTest extends AbstractSqlTest {
    ...
    public void testStr() {
        testSqlForMany("SELECT ascii(s),len(s),CHAR_LENGTH(s),CHARACTER_LENGTH(s),concat(s1,s2),concat(s1,s2,s3),group_concat(a,b),group_concat(distinct a,b),group_concat(distinct a,b order by a separator ','),concat_ws(s,s1,s2,s3),repeat(s,n),reverse(s),replace(s,from_s,to_s)" +
